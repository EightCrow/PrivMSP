...
class SoftPromptMixDataset(BaseDataset):
  ...
  def __init__(self, args, examples, tokenizer, split):
    ...
    self.use_lora = hasattr(args, 'privacy_tune') and args.privacy_tune
  ...
  def collate_seq2seq(self, args, examples):
    ...
    # 如果使用 LoRA，不需要添加额外的 soft prompt tokens
    if not self.use_lora:
      ...
      attention_mask = torch.ones((batch_size, args.n_tokens), dtype=datatype).to(device)
      inputs['attention_mask'] = torch.cat([attention_mask, inputs['attention_mask']], dim=1)

    # 添加标签
    if self.split == 'train':
      label_tokens = self.tokenizer(labels, padding=True, max_length=args.target_max_len,
                                  truncation=True, return_tensors='pt').to(device)
      inputs['labels'] = label_tokens['input_ids']
      inputs['decoder_input_ids'] = self._shift_right(label_tokens['input_ids'])
