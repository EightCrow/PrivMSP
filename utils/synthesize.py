...
def filter_generated_data(args, new_data, synthetic_data, seed_data):
  """
  过滤生成的数据，使用传统方法或改进的参数化差分隐私去噪过滤算法
  ...
  if args.advanced_filter:
    ...
    if args.visualize_distribution:
      ...
      visualize_distributions(
        distribution_info, 
        save_path=os.path.join(args.visualization_dir, "cluster_distribution.png")
      )
      ...
      visualize_attribute_distribution(
        distribution_info, 
        top_n=10, 
        save_path=os.path.join(args.visualization_dir, "attribute_distribution.png")
      )
      
      print(f"分布可视化已保存到 {args.visualization_dir}")
    ...
    # 选择噪声分数最低的样本
    num_to_keep = min(args.target_samples, len(flat_data)) if hasattr(args, 'target_samples') else len(flat_data) // 2
    filtered_indices = sorted_indices[:num_to_keep]
    
    # 获取过滤后的数据
    filtered_data = [flat_data[i] for i in filtered_indices]
